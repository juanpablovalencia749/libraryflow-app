...
export const BookCard = ({ book, onAction }: BookCardProps) => {
  ...
  const canLoanNow =
    (reservationCount === 0 && isAvailable) ||
    (hasMyReservation &&
      myQueuePosition === 1 &&
  ...
  return (
    <Card
      className={`flex flex-col overflow-hidden hover:shadow-lg transition-all duration-300 border-none bg-white dark:bg-gray-800/50 backdrop-blur-sm group ${borrowedByMe ? "ring-2 ring-indigo-500/50" : hasMyReservation ? "ring-2 ring-emerald-500/50" : ""}`}
    >
      <Link
        to={`/books/${book.id}`}
      ...
      >
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
          ...
          {hasMyReservation && (
            <Badge
              className={`border-none font-bold tracking-wider uppercase shadow-md animate-in slide-in-from-right-2 ${myQueuePosition === 1 && isAvailable ? "bg-green-600 text-white" : "bg-emerald-600 text-white"}`}
            >
              {myQueuePosition === 1 && isAvailable
                ? "Your Turn!"
                : `Waitlist #${myQueuePosition}`}
            </Badge>
          )}
          ...
          {(reservationCount ?? 0) > 0 && (
            <Badge
              variant="outline"
              className="bg-white/90 dark:bg-gray-900/90 gap-1 font-bold"
            >
      ...
      <CardContent className="p-5 pt-0 mt-auto space-y-3">
          ...
          >
            <span className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-black/5 dark:bg-white/10 px-1.5 py-0.5 rounded-md">
                Next: #
                {nextQueuePositionIfReserveNow ?? (reservationCount ?? 0) + 1}
              </span>
            </span>
          </Button>
        )}
        ...
        <Link to={`/books/${book.id}`} className="block">
          <Button
            variant="outline"
            className="w-full rounded-xl gap-2 h-10 border-gray-200 dark:border-gray-700 shadow-sm hover:bg-gray-50 dark:hover:bg-gray-800"
          >
